...
export default class Item {
  constructor(item = {}) {
    ...
    if (!item.copies) {
      this.copies = [];
    } else if (Array.isArray(item.copies)) {
      this.copies = item.copies.map(copy => new Copy(copy));
    } else {
      this.averagePrice = Math.round(item.copies.averagePrice);
      this.copies = [];
      this.inStock = item.copies.quanity;
    }
  }
